...
namespace Command
{
    public interface ICommand
    {
        bool Qable { get; }
        void Execute();
        void SetReceiver(Receiver receiver) { } //default, because some commands don't have receiver
    }
    ...
    public class EditCommand : FindCommand
    {
        ...
        private List<(string property, object value)> _old_settings;
        ...
        public override void Execute()
        {
            _item = _receiver.Edit(name, parameters, settings, out _old_settings);
        }
        public void Undo() //NOTE: this method is not supported after deserialization
        {
            var properties = new List<PropertyInfo>(_item.GetType().GetProperties());
            foreach ((string propertyName, object value) in _old_settings)
            {
                var property = properties.Find((PropertyInfo x) => x.Name == propertyName)!;
    ...
    {
        public bool Qable => false;
        protected Stack<ICommand> _history;
        public QueueCommand(Stack<ICommand> queue)
        {
            _history = queue;
        }
    ...
    {
        public QueuePrint(Stack<ICommand> queue) : base(queue) { }
        public override void Execute()
        {
    ...
    public class QueueExport : QueueCommand
    {
        ...
        private bool _toXML;
        public QueueExport(Stack<ICommand> queue, string[] arguments) : base(queue)
        {
            _path = arguments[0];
            _toXML = true;
        ...
        public override void Execute()
        {
            try
            {
                ...
                if (_toXML)
                {
                    var commands = new List<object>(_history);
                ...
                else
                {
                    ...
                    foreach (ICommand command in _history) writer.WriteLine(command.ToString());
                }
        ...
        public override string ToString()
        {
            string format = _toXML ? "xml" : "plaintext";
            return $"export {_path} {format}";
        }
    }
    public class QueueLoad : QueueCommand
    {
        ...
        public QueueLoad(Stack<ICommand> queue, string path, CommandCreator commandCreator, Receiver receiver) : base(queue)
        {
    ...
    public class Invoker
    {
        private readonly CommandCreator _commandCreator;
        private readonly Receiver _receiver;
        private readonly QueuePrint _printer;
        private Stack<ICommand> _history;
        private Stack<ICommand> _undone;
        public Invoker(Receiver receiver)
        {
            _commandCreator = new CommandCreator(receiver);
            _receiver = receiver;
            _history = new Stack<ICommand>();
            _undone = new Stack<ICommand>();
            _printer = new QueuePrint(_history);
        }
        ...
        private void Undo()
        {
            ...
            _receiver.Undo();
            if (command is EditCommand edit) edit.Undo();
        }
        private void Redo()
        {
            ...
            _receiver.Redo();
            if (command is EditCommand edit) edit.Execute();
        }
        public void Run()
        {
            ...
            while (true)
            {
                ...
                try
                {
                    //switch isn't extensible, but I don't have many time and it works
                    switch (frazes[0])
                    {
                        case "export":
                            if (frazes.Length > 1)
                            {
                                command = new QueueExport(_history, frazes[1..]);
                                command.Execute();
                                command = null;
                            }
                        ...
                        case "import":
                            if (frazes.Length > 1)
                            {
                                command = new QueueLoad(_history, frazes[1], _commandCreator, _receiver);
                                command.Execute();
                                command = null;
                            }
                            else Console.WriteLine("Nie znaleziono podanej komendy.");
    ...
    public class Receiver
    {
        ...
        private readonly Dictionary<string, List<(string, Type)>> _properties; //could be replaced by PropertyInfo, offering informations such as Name or PropertyType
        private readonly Dictionary<string, IBuilder> _builders;
        private IMyCollection<object> _collection;
        //I know how it looks like, but trust me, it's the easiest way to do it
        private Stack<IMyCollection<object>> _history;
        private Stack<IMyCollection<object>> _undone;
        public Receiver(IMyCollection<object> collection, Dictionary<string, Type> types, Dictionary<string, List<(string, Type)>> properties, Dictionary<string, IBuilder> builders)
        {
            ...
            _builders = builders;
            _history = new Stack<IMyCollection<object>>();
            _undone = new Stack<IMyCollection<object>>();
            Save(_collection);
        }
        private void Save(IMyCollection<object> collection)
        {
            var new_collection = (IMyCollection<object>)Activator.CreateInstance(_collection.GetType())!;
            MyCollectionAlgorithms.ForEach(collection.GetForwardBegin, (object x) => { new_collection.Add(x); });
            _history.Push(new_collection);
        }
        public void Undo()
        {
            if (_history.Count == 1) return;
            _undone.Push(_history.Pop());
            _collection = _history.Peek();
        }
        public void Redo()
        {
            if (_undone.Count == 1) return;
            _history.Push(_undone.Pop());
            _collection = _history.Peek();
        }
        ...
        public object Edit(string name, (string, object, char)[] parameters, List<(string property, object value)> settings, out List<(string property, object value)> old_settings)
        {
            Type type = _types[name];
            var properties = new List<PropertyInfo>(type.GetProperties());
            ...
            {
                var property = properties.Find((PropertyInfo x) => x.Name == propertyName)!;
                old_settings.Add((propertyName, property.GetValue(item)!));
